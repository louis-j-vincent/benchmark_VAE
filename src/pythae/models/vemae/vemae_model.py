...
### update parameters
    ...
    def data_augmentation(self, x, y, z):
        """
        ...
        Y = y.repeat_interleave(n_repeats, dim=0)
        XV = self.corrupt(X) 
        V = (XV==-10).float()
...
### losses
    ...
    def temporal_loss_VFEMAE(self,Z,X,tempo):
        """
        Temporal loss
    ...
    def temporal_loss(self,Z,X,tempo):
        ...
        return torch.abs( delta_Z.mean(axis=-1) * delta_U.sum(axis=-1) / (delta_X * delta_U).sum(axis=-1) ).mean()
...
### forward
    ...
    def forward(self, inputs: BaseDataset, **kwargs) -> ModelOutput:
        ...
        # get data
        x, y, tempo, v = self.get_inputs(inputs)

        # get latent representation z
        z = self.encoder(x).embedding

        # augment data
        X, XV, Y, Z, V = self.data_augmentation(x, y, z)

        # get corrupt latent representation ZV
        ZV = self.encoder(XV).embedding

        ## separate in 2 parts, the one decorrelated from mfgs or v (with prior) and the one without
        ...
        hsic_loss = self.HSIC(Z_prior, V, s_x=1, s_y=1, sample_size = 200)
        #hsic_loss = torch.tensor(0)
        ...
        loss += (LLloss + classif_loss)*self.beta
